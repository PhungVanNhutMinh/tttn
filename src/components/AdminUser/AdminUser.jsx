...
const AdminUser = () => {
    ...
    const dataTable = users?.data?.length && Array.isArray(users?.data) ? users?.data?.map((user) => {
    ...
    return (
        <div>
            ...
            <DrawerComponent title='Chi tiết người dùng' isOpen={isOpenDrawer} onClose={() => setIsOpenDrawer(false)} width='90%'>
                <Loading isLoading={isLoadingUpdate || isLoadingUpdate}>
                    ...
                    >
                        ...
                        >
                            <InputComponent value={stateUserDetails.name} onChange={handlOnchangeDetails} name="name" />
                        </Form.Item>
                        ...
                        >
                            <InputComponent value={stateUserDetails['email']} onChange={handlOnchangeDetails} name="email" />
                        </Form.Item>
                        ...
                        >
                            <InputComponent value={stateUserDetails.phone} onChange={handlOnchangeDetails} name="phone" />
                        </Form.Item>
                        ...
                        >
                            <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                                <WrapperUploadFile
                                    ...
                                    maxCount={1}
                                    beforeUpload={() => false}
                                    fileList={stateUserDetails.image ? [{ uid: '-1', name: 'image', status: 'done', url: stateUserDetails.image }] : []} // Hiển thị ảnh đã chọn lên UI của Upload
                                >
            ...
            <ModalComponent forceRender title="Xóa người dùng" open={isModalOpenDelete} onCancel={handleCancelDelete} onOk={handlDeleteUser}>
                <Loading isLoading={isPendingDeleted}>
                    <div>Bạn có chăc mình muốn xóa tài khoản này không?</div>
                </Loading>
            </ModalComponent>
        </div>
    )
